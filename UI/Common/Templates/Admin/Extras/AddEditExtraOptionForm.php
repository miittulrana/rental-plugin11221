...
<div id="container-inside">
  <span class="title">Add/Edit Extra Option</span>
  <input type="button" value="Back to Extra Options List" onclick="window.location.href='<?=esc_js($backToListURL);?>'" class="button-back"/>
    <hr />
  <form action="<?=esc_url($formAction);?>" method="POST" id="form1">
    <table cellpadding="5" cellspacing="2" border="0">
        <input type="hidden" name="option_id" value="<?=esc_attr($optionId);?>"/>
        <tr>
            <td width="20%"><strong>Select an Extra:<span class="is-required">*</span></strong></td>
            <td width="80%">
                <select name="extra_id" class="required">
                    <?=$trustedExtrasDropdownOptionsHTML;?>
                </select>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td>
                <input type="submit" value="Save extra option" name="save_option" class="save-button"/>
            </td>
        </tr>
    </table>
  </form>
