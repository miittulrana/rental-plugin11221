...
?>
<div class="fleet-management-extras-availability-calendar <?=esc_attr($extCSS_Prefix);?>extras-availability-calendar">
    <div class="calendar-label">
        <?php
        if($extrasAvailabilityCalendar['30_days']):
            print(esc_html($lang['LANG_EXTRAS_AVAILABILITY_IN_NEXT_30_DAYS_TEXT']));
        else:
            print(esc_html($lang['LANG_EXTRAS_AVAILABILITY_FOR_TEXT']).' '.$extrasAvailabilityCalendar['print_month_name'].', '.$extrasAvailabilityCalendar['print_year']);
        endif;
        ?>
    </div>
    <hr/>
        ...
        <?php foreach($extrasAvailabilityCalendar['extras'] AS $extra): ?>
            <tr class="extra-row">
                <td class="extra-details">
                    <span class="extra-name"><?=$extra['extra_name_with_dependant_item_model'];?></span>
                    <br /><hr />
                    <?=esc_html($lang['LANG_EXTRA_ID_TEXT']);?>: <?=esc_html($extra['extra_id']);?>
