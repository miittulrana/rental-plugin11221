...
<div class="car-rental-list-admin calendar-search-results fleet-management-tabbed-admin <?=esc_attr($extCSS_Prefix);?>tabbed-admin fleet-management-tabbed-admin-wide <?=esc_attr($extCSS_Prefix);?>tabbed-admin-wide bg-cyan">
	<div class="body">
		...
		<div class="modern-tabs modern-tabs-pos-top-left modern-tabs-anim-flip modern-tabs-response-to-icons">
			...
			<label for="modern-tab1"><span><span><i class="fa fa-car" aria-hidden="true"></i>Available <?=esc_html($lang['LANG_MULTIPLE_VEHICLE_TITLE_UPPERCASE']);?></span></span></label>
			<ul>
				<li class="modern-tab-content-1">
					<div class="typography">
						...
						<div class="col-search">
							<input class="back-to" type="button" value="Back to This Month <?=esc_html($lang['LANG_SINGLE_VEHICLE_TITLE_UPPERCASE']);?> Calendar"
								onclick="window.location.href='<?=esc_js($backToCurrentAvailabilityURL);?>'"
								/>
						</div>
						<?php foreach($arrItemModelsAvailabilityCalendars AS $itemModelsAvailabilityCalendar): ?>
							<?php
							...
							?>
						<?php endforeach; ?>
						<?php if(sizeof($arrItemModelsAvailabilityCalendars) == 0): ?>
