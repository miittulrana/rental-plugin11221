...
final class PartnersObserver implements ObserverInterface
{
    private $conf           = null;
    private $lang 		    = null;
    ...
    public function getTrustedDropdownOptionsHTML($paramSelectedWP_UserId = -1, $paramDefaultValue = -1, $paramDefaultLabel = "")
    {
        ...
        $roleName = (new PartnerRole($this->conf, $this->lang))->getRoleName();
        ...
        foreach($arrObjWP_Users AS $objWP_User)
        {
            ...
            {
                $retHTML .= '<option value="'.esc_attr($validWP_UserId).'" selected="selected">'.esc_html($objWP_User->display_name).'</option>';
            } else
            {
                $retHTML .= '<option value="'.esc_attr($validWP_UserId).'">'.esc_html($objWP_User->display_name).'</option>';
            }
        }
        return $retHTML;
    }
