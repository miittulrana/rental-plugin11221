...
final class ExtraDiscount extends AbstractStack implements ElementInterface
{
    protected $conf 	                = null;
    protected $lang 		            = null;
    ...
    public function save(array $params)
    {
        ...
        $validExtraId = isset($params['extra_id']) ? StaticValidator::getValidPositiveInteger($params['extra_id'], 0) : 0;
        $validDiscountType = isset($params['discount_type']) && $params['discount_type'] == 4 ? 4 : 3;
        ...
        {
            $ok = false;
            $this->errorMessages[] = $this->lang->getText('LANG_EXTRA_DISCOUNT_DAYS_INTERSECTION_ERROR_TEXT');
        }
        ...
        if($validDiscountId > 0 && $ok)
        {
            ...
            if($saved === false)
            {
                $this->errorMessages[] = $this->lang->getText('LANG_EXTRA_DISCOUNT_UPDATE_ERROR_TEXT');
            } else
            {
                $this->okayMessages[] = $this->lang->getText('LANG_EXTRA_DISCOUNT_UPDATED_TEXT');
            }
        } else if($ok)
        {
            $insertSQL = "
            ...
            {
                $this->errorMessages[] = $this->lang->getText('LANG_EXTRA_DISCOUNT_INSERTION_ERROR_TEXT');
            } else
            {
                $this->okayMessages[] = $this->lang->getText('LANG_EXTRA_DISCOUNT_INSERTED_TEXT');
            }
        }
    ...
    public function delete()
    {
        ...
        {
            $this->errorMessages[] = $this->lang->getText('LANG_EXTRA_DISCOUNT_DELETION_ERROR_TEXT');
        } else
        {
            $this->okayMessages[] = $this->lang->getText('LANG_EXTRA_DISCOUNT_DELETED_TEXT');
        }
