...
final class Attribute2 extends AbstractStack implements ElementInterface
{
    protected $conf 	                = null;
    protected $lang 		            = null;
    ...
    private function getDataFromDatabaseById($paramAttributeId)
    {
        ...
        $retData = $this->conf->getInternalWPDB()->get_row("
            SELECT transmission_type_id AS attribute_id, transmission_type_title AS attribute_title, blog_id
            FROM {$this->conf->getPrefix()}transmission_types
            WHERE transmission_type_id='{$validAttributeId}'
        ", ARRAY_A);
    ...
    public function getDetails($paramPrefillWhenNull = false)
    {
        ...
        if(!is_null($ret))
        {
            ...
            $ret['translated_attribute_title'] = $this->lang->getTranslated("at2_{$ret['attribute_id']}_attribute_title", $ret['attribute_title']);
    ...
    public function save(array $params)
    {
        ...
        $titleExistsQuery = "
            ...
            FROM {$this->conf->getPrefix()}transmission_types
            WHERE transmission_type_title='{$validAttributeTitle}' AND transmission_type_id!='{$validAttributeId}'
        ...
        if($validAttributeId > 0 && $ok)
        {
            $saved = $this->conf->getInternalWPDB()->query("
                ...
                transmission_type_title='{$validAttributeTitle}'
                WHERE transmission_type_id='{$validAttributeId}' AND blog_id='{$this->conf->getBlogId()}'
            ");
    ...
    public function registerForTranslation()
    {
        ...
        {
            $this->lang->register("at2_{$this->attributeId}_attribute_title", $attributeDetails['attribute_title']);
